...
def extend_for_footprints(Constants, n):
    return n + n // Constants.BATCH_SIZE

def determine_conv_tile_c(onnx_model: onnx.ModelProto, config: dict[str, Any], Constants, n):
...
def determine_gemm_tile_sizes(onnx_model: onnx.ModelProto, config: dict[str, Any], Constants, n):
    ...
    while True:
        ...
        full_tile_width = (extend_for_footprints(Constants, tile_size_unit)+1)/2*2
